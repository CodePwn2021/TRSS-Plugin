...
if (process.platform == 'win32') { prompt = cmd => `powershell -EncodedCommand ${Buffer.from(`$ProgressPreference="SilentlyContinue";[Console]::OutputEncoding=[System.Text.Encoding]::UTF8;[Console]::Write("$(prompt)"+'${cmd.replace(/'/g, '\'+"\'"+\'')}\n');${cmd}`, 'utf-16le').toString('base64')}` }
...
export class RemoteCommand extends plugin {
  ...
  async JS (e) {
    ...
    if (ret.error) { await this.reply(`错误输出：\n${ret.error.stack}`, true) }
  }
  ...
  async JSPic (e) {
    ...
    let Code = []
    if (ret.stdout) { Code.push(ret.stdout.trim()) }
    if (ret.error) { Code.push(`错误输出：\n${Bot.Loging(ret.error)}`) }
  ...
  async ShellPic (e) {
    ...
    let Code = []
    if (ret.stdout) { Code.push(ret.stdout.trim()) }
    if (ret.error) { Code.push(`远程命令错误：\n${Bot.Loging(ret.error)}`) } else if (ret.stderr) { Code.push(`标准错误输出：\n${ret.stderr.trim()}`) }

    Code = await ansi_up.ansi_to_html(Code.join('\n\n'))
    Code = cmdColoring(Code)
    const img = await puppeteer.screenshot('Code', { tplFile, htmlDir, Code })
    return this.reply(img, true)
  }
  ...
  async DirectMsg () {
    ...
    if (ret.error) { return this.reply(`错误输出：\n${ret.error.stack}`, true) }
    try {
      const m = []
  ...
  async MultiMsg () {
    ...
    try {
      ...
      const r = []
      for (const i of m) { r.push(await this.reply(i)) }
      return r
    } catch (err) {
      return this.reply(`错误输出：\n${error.stack}`, true)
    }
